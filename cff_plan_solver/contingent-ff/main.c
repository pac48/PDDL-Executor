...
void output_planner_info(void) {
    ...
    printf("\n               final representation has %d unknown facts and %d observation actions    ",
           ginitial_state.num_U, count_observe_actions);
    ...
    printf("\n            %7.2f seconds total time (+ %7.2f secs for CNF memory allocation)",
           gtempl_time + greach_time + grelev_time + gconn_time + genc_time + gsearch_time + gsat_time + geval_time +
           gr_sat_time + grp_sat_time + gr_cnf_time + gr_enc_time + gcnf_time + grs_time + gss_time + grs_sat_time,
           gmem_time);
